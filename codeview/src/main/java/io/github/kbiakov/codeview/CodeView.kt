...
open class CodeView : RelativeLayout {
    //open class CodeView(context: Context,attrs:AttributeSet) : RelativeLayout(context, attrs) {
//lateinit表明 可以直接初始化，而不经过什么 抽象啊啥的
    private lateinit var vCodeList: RecyclerView;
    private lateinit var vShadows: Map<ShadowPosition, View>;
    ...
    open protected  fun doInit(context: Context, attrs: AttributeSet?) {//?允许空

        inflate(context, R.layout.layout_code_view, this)
        checkStartAnimation(attrs)//强制传递为空进去。因为在里面检查了。 但是这里也要写上?代表允许空，不然 !!写上后  Caused by: kotlin.KotlinNullPointerException
...
        constructor(context: Context, attrs: AttributeSet?, defStyleAttr: Int) : super(context, attrs, defStyleAttr)
    ...
    private fun checkStartAnimation(attrs: AttributeSet?) {
        if (visibility == VISIBLE && attrs != null && attrs.isAnimateOnStart(context)) {
