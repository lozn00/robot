...
import cn.qssq666.robot.plugin.js.api.JSPluginInterface;
...
public class JSPluginUtil {
    private static final String TAG = "JSPluginUtils";

    private static FileFilter getPluginFileFilter() {
        return new JSPluginFilter();
    }

    static class JSPluginFilter implements FileFilter {
        ...
        public boolean accept(File pathname) {
            ...
            boolean result = absolutePath.endsWith(".js")
                    || ((absolutePath.endsWith(".txt") && (pathname.getName().contains("js") || pathname.getName().contains("javascript"))));
            return result;
        }
    }
    ...
    public static List<QueryPluginModel> loadPluginFromPath(Context context, ClassLoader classLoader, File path, OnCareteNotify onCreateNotify) {
        File[] files = path.listFiles(new JSPluginFilter());
        ...
        if (files != null) {
            for (int i = 0; i < files.length; i++) {
                ...
                model.setPath(myFile.getAbsolutePath());
                try {
                    PluginInterface pluginInterface = new JSPluginInterface(myFile);
                    ...
                    if (BuildConfig.DEBUG) {
                        ...
                        Log.w(TAG, "加载插件" + myFile.getAbsolutePath() + "成功");
                    }

                    RobotContentProvider.getInstance().writePluginErrorLog(myFile, null);
                } catch (Throwable e) {

                    if (e instanceof ClassNotFoundException) {
                        e.printStackTrace();
                        RobotContentProvider.setLastError(e);
                        String msg = "加载" + myFile.getName() + "文件插件失败,ClassNotFoundException，导致找不到" + Cns.PLUGIN_MAIN_ENTRY_FILE + "类";
                        Log.e(TAG, msg);
                        ...
                        Log.e(TAG, "加载js插件失败,未知异常 " + e.getMessage());

                    } else if (e instanceof Error) {
                        RobotContentProvider.setLastError(e);
                        e.printStackTrace();
                        Log.e(TAG, "加载JS插件失败,未知错误 " + e.getMessage());
                    ...
                    RobotContentProvider.getInstance().writePluginErrorLog(myFile, jsonObject.toString());
                }
            }
...
/*
    ...
    public static List<QueryPluginModel> loadPlugin(Context context, ClassLoader classLoader, PluginUtils.OnCareteNotify onCareteNotify) {
        ...
        return loadPluginFromPath(context, getDefaultPath(context), classLoader);
    }
*/
    ...
    public static File getDefaultPath(Context context) {
        File pluginDir = new File(RobotUtil.getBaseDir(), "robot_plugin_js");
