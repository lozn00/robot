...
public class QQIgnoresActivity extends BaseAccountManagerActivity<AccountBean> {
    ...
    protected long onInsertToDb(AccountBean accountBean) {
        return DBHelper.getIgnoreQQDBUtil(AppContext.dbUtils).insert(accountBean);
    ...
    protected int deleteAllFromDb() {
        return DBHelper.getIgnoreQQDBUtil(AppContext.dbUtils).deleteAll(AccountBean.class);
    }
    ...
    protected int onDeleteToDb(AccountBean bean) {
        int i = DBHelper.getIgnoreQQDBUtil(AppContext.getDbUtils()).deleteById(AccountBean.class, bean.getId());
        return i;
    }

    protected List<AccountBean> queryDataFromDb() {
        List<AccountBean> list = DBHelper.getIgnoreQQDBUtil(AppContext.getDbUtils()).queryAllIsDesc(AccountBean.class, true, FieldCns.FIELD_ACCOUNT);
        return list;
    }
