...
import cn.qssq666.robot.ide.mycode.common.language.LanguageNonProg;
...
public class Lexer {
    ...
    public static Language get_globalLanguage() {
    ...
    void tokenizeDone(List<MyPair> result) {
        if (_callback != null) {
    ...
    private class LexThread extends Thread {
         ...
         */
        private ArrayList<MyPair> _tokens;
        ...
        private void tokenize() {
            ...
            ArrayList<MyPair> tokens = new ArrayList<MyPair>(8196);
            LexerI lexer = null;
            ...
            try {
                if (language instanceof LanguageJavascript) {
                    lexer = new LuaLexer(hDoc);
//                    lexer = new JSLexer(hDoc);
                } else {
                    lexer = new LuaLexer(hDoc);
                }
                int idx = 0;
                ...
                while (!_abort.isSet()) {
                    ...
                    if (lastType2 == type && lastPair != null) {
                        ...
                        //关键字
                        tokens.add(new MyPair(len, KEYWORD));
                        ...
                        //括号
                        tokens.add(pair = new MyPair(len, OPERATOR));
                    } else if (type == LuaTokenTypes.STRING || type == LuaTokenTypes.LONGSTRING) {
                        //字符串
                        if (lastType != type) {
                            tokens.add(pair = new MyPair(len, SINGLE_SYMBOL_DELIMITED_A));
                        ...
                        if (lastType == LuaTokenTypes.FUNCTION) {
                            //函数名
                            tokens.add(new MyPair(len, LITERAL));
                            language.addUserWord(name);
                        } else if (language.isUserWord(name)) {
                            tokens.add(new MyPair(len, LITERAL));
                        } else if (language.isBasePackage(name)) {
                            tokens.add(new MyPair(len, NAME));
                        } else if ((lastType == LuaTokenTypes.DOT || language.isMethodAppendToken(lastType)) && language.isBasePackage(lastName) && language.isBaseWord(lastName, name)) {
                            //标准库函数
                            tokens.add(new MyPair(len, NAME));
                        }
                        ...
                        //注释
                        if (lastType != type)
                            tokens.add(pair = new MyPair(len, DOUBLE_SYMBOL_LINE));
                        else
                            lastPair.setFirst(lastPair.getFirst() + len);
                    } else if (type == LuaTokenTypes.NUMBER) {
                        tokens.add(new MyPair(len, LITERAL));
                    } else {
                        tokens.add(pair = new MyPair(len, NORMAL));
                    }
            ...
            if (tokens.isEmpty()) {
                ...
                tokens.add(new MyPair(0, NORMAL));
            }
        ...
        public void tokenize2() {
            DocumentProvider hDoc = getDocument();
            Language language = Lexer.getLanguage();
            ArrayList<MyPair> tokens = new ArrayList<MyPair>();
...
//TODO consider less greedy approach and avoid adding token for previous char
                            ...
                            if (currentCharInWord > 0 && state != NORMAL) {
                                tokens.add(new MyPair(workingPosition - currentCharInWord, NORMAL));
                            }

                            state = pendingState;
                            tokens.add(new MyPair(spanStartPosition, state));
                            currentCharInWord = 0;
                        } else if (language.isWhitespace(currentChar) || language.isOperator(currentChar) || language.isOperator(prevChar + "" + currentChar)) {
                            ...
                            if (state != NORMAL && language.isOperator(currentChar) || language.isOperator(prevChar + "" + currentChar)) {
                                state = NORMAL;
                                tokens.add(new MyPair(workingPosition, state));
                            }
    ...
    public interface LexCallback {
        public void lexDone(List<MyPair> results);
    }
}
//ignore_end
