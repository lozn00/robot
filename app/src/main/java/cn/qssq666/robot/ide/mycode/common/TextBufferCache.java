...
public class TextBufferCache {
	...
	private MyPair[] _cache = new MyPair[CACHE_SIZE];
	
	public TextBufferCache(){
		_cache[0] = new MyPair(0, 0); // invariant lineIndex and charOffset relation
		for (int i = 1; i < CACHE_SIZE; ++i){
			_cache[i] = new MyPair(-1, -1);
	...
	public MyPair getNearestLine(int lineIndex){
		int nearestMatch = 0;
		...
		MyPair nearestEntry = _cache[nearestMatch];
		makeHead(nearestMatch);
		return nearestEntry;
	}
	
	public MyPair getNearestCharOffset(int charOffset){
		int nearestMatch = 0;
		int nearestDistance = Integer.MAX_VALUE;
		for(int i = 0; i < CACHE_SIZE; ++i){
	...
	private void insertEntry(int lineIndex, int charOffset){
		...
		_cache[1] = new MyPair(lineIndex, charOffset);
	}
